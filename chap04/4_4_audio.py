...
class ArrayLayer(TiledLayer):
    def __init__(self, tileSize, imageFile, gameState, array, surfaceFlags=pygame.SRCALPHA):
        super().__init__(tileSize, imageFile)
...
class UnitsLayer(TiledLayer):
    def __init__(self, tileSize, imageFile, gameState, units):
        super().__init__(tileSize, imageFile)
        self.state = gameState
        self.units = units
...
class BulletsLayer(TiledLayer):
    def __init__(self, tileSize, imageFile, gameState, bullets):
        super().__init__(tileSize, imageFile)
...
class MenuGameMode(GameMode):
    def __init__(self):
        ...
        self.menuItems = [
            {
                'title': 'Level 1',
                'action': lambda: self.notifyLoadLevelRequested("Level 1.tmx")
            },
            {
                'title': 'Level 2',
                'action': lambda: self.notifyLoadLevelRequested("Level 2.tmx")
            },
            {
                'title': 'Level 3',
                'action': lambda: self.notifyLoadLevelRequested("donotexists.tmx")
            },
            {
                'title': 'Quit',
...
class PlayGameMode(GameMode):
    def loadLevel(self, fileName):
        ...
        self.tileSize = tileSize = loader.tileSize
        self.renderWidth = state.worldSize[0] * tileSize[0]
        self.renderHeight = state.worldSize[1] * tileSize[1]
        
        # Layers
        self.layers = [
            ArrayLayer(tileSize, loader.groundTileset, state, state.ground, 0),
            ArrayLayer(tileSize, loader.wallsTileset, state, state.walls),
            UnitsLayer(tileSize, loader.unitsTileset, state, state.units),
            BulletsLayer(tileSize, loader.bulletsTileset, state, state.bullets),
            ExplosionsLayer(tileSize, loader.explosionsTileset),
            SoundLayer("170274__knova__rifle-fire-synthetic.wav",
                       "110115__ryansnook__small-explosion.wav")
        ]
